...
const Product = () => {
    ...
    const { productData, cartId } = useContext(Store)
    ...
    const handleAddToCart = async ({ name, id, price }) => {
        for (let index = 0; index < cartId.length; index++) {
            const element = cartId[index];
            if (cartId && element && element === id) {
                console.log(id);
                return
            } else {
    ...
    return (
        <div class="font-[sans-serif]">
            ...
            <h2 className="text-gray-700 p-2 font-bold text-3xl"> New Arrivals: </h2>
                            ...
                            return (
                                <>
                                    <div class="bg-gray-100 rounded-2xl p-6 cursor-pointer hover:-translate-y-2 transition-all relative">
                                        ...
                                        <div onClick={() => handleDetail({ id: product.id })} class="max-lg:w-11/12 w-4/5 h-[220px] overflow-hidden mx-auto aspect-w-16 aspect-h-8">
